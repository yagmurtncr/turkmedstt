"""Single-encoder multi-head token editor model."""
...
class MultiTaskTokenEditorConfig(PretrainedConfig):
    ...
    def __init__(
        self,
        encoder_name: str = "ytu-ce-cosmos/turkish-mini-bert-uncased",
        encoder_config: dict | None = None,
        num_case_labels: int = 3,
        num_punct_labels: int = 8,
        num_edit_labels: int = 2,
        **kwargs,
    ):
...
class MultiTaskTokenEditor(PreTrainedModel):
    ...
    def __init__(self, config: MultiTaskTokenEditorConfig):
    ...
    @classmethod
    def from_encoder_pretrained(cls, encoder_name: str, **kwargs):
        encoder_config = AutoConfig.from_pretrained(encoder_name)
        config = MultiTaskTokenEditorConfig(
            encoder_name=encoder_name,
            encoder_config=encoder_config.to_dict(),
            **kwargs,
        )
        model = cls(config)
        model.encoder = AutoModel.from_pretrained(encoder_name, config=encoder_config)
        return model
    ...
    def forward(
        ...
        **kwargs,
        ...
        return MultiTaskTokenEditorOutput(loss=loss, case_logits=case_logits, punct_logits=punct_logits, edit_logits=edit_logits)
